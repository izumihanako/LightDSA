...
int COPY_LEN = 512 , method = 0 , tdesc = 32768 , bsiz = 64 ;
...
void test_dsa_batch( vector<OffLen> test_set , char *a , char* b , int method  ){ 
    ...
    if( method == 0 ){
        ...
        for( int tmp = 0 ; tmp < REPEAT ; tmp ++ ){ 
            st_time = timeStamp_hires() ;  
            for( int i = 0 ; i < cnt ; i ++ )
                memcpy( b + test_set[i].off_dest , a + test_set[i].off_src , test_set[i].len ) ;
            ed_time = timeStamp_hires() , do_time = ed_time - st_time , st_time = ed_time ; 
            dsa_time += ( do_time ) / REPEAT ;
        } 
    }
    ...
    else if( method == 2 ) printf( "    ; memcpy     cost %5.2lfus, speed %5.0fMB/s\n" , dsa_time , dsa_speed ) ; 
    puts( "" ) ;
} 
...
int main( int argc , char *argv[] ){
    if( argc > 4 ){
        ...
        printf( "method    : 0 DSA_batch, 1 DSA_single, 2 memcpy\n" ) ;
        printf( "array_len : KB\n" ) ;
...
// python3 perf_PIPE.py "./expr/cache/same_desc_diff_mem 1 1024 4096 400 1"
// "Translation hits" 的数量约等于 tdesc 的数量
// 说明如果desc相同，但src和dest不同，可以触发ATC hit

// 对于batch操作，desc list, comp addr分别需要一次访存，共两次