...
public: 
    ...
    void run() noexcept(true) { 
        ...
        for( int i = 0 ; i < tdesc ; i ++ ){
            batch_.submit_comp_pattern( (void*)((uintptr_t) dest + ( 1LL * i * len ) % arr_len ) , pattern , len ) ;
        }  
        yield_out() ; 
        batch_.wait() ; 
        return ;
...
static inline void cmpval(char *buf, uint64_t len)
{
...
int main(){   
    ...
    for( size_t i = 0 ; i < ARRAY_LEN ; i ++ ) a[i] = 0 ; 
    ...
    for( size_t len = 64 , warmup = 0 ; len <= 4 * MB ; len *= 2 ){ 
        ...
        for( int tmp = 0 ; tmp < REPEAT ; tmp ++ ){
            ...
            for( int i = 0 ; i < tdesc ; i ++ ) cmpval( a + ( 1LL * i * len ) % ARRAY_LEN , len ) ; 
        ...
        for( int tmp = 0 ; tmp < REPEAT ; tmp ++ ){ 
            xfer.db_task.clear() ;
            st_time = timeStamp_hires() ;
            for( int i = 0 ; i < tdesc ; i ++ ) xfer.submit_comp_pattern( a + ( 1LL * i * len ) % ARRAY_LEN , 0 , len ) ;
